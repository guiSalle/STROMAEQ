...
fec = fec[!(fec$VERS %in% c('Anoplocephale_opg','Anoplocephales_vus','oxyures',
                            'OXYURES','oxyures prEsence(1) absence(0)','P_equorum_opg','P_equorum_vus','PASCARIS')),]
dim(fec)
#[1] 4596   10
...
ggplot(fecToPlot, aes(x = Time, y = RESULTATS_OPG, fill = Group)) +
  geom_point(aes(x = Time, y = RESULTATS_OPG,col = Group),
  ...
  coord_flip() + ylab('Faecal Egg Count (eggs/g)') +
  xlab('') + theme_classic() +
...
factor(names(feat_mat[,'Monocytes'][abs(feat_mat[,'Monocytes']) >= 0.3]))
